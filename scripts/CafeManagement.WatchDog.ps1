...
$ChromeTitlePatterns = @("LAN Cafe Ops", "CafeManagement")
...
function Test-ChromeCommandLineHasCafeManagement {
    try {
        ...
        foreach ($process in $chromeProcesses) {
            if ($process.CommandLine -and $process.CommandLine.Contains($TargetUrlToken)) {
                Write-Log "CafeManagement page detected by Chrome command line. PID: $($process.ProcessId)"
                return $true
            }
        }
    }
...
function Test-ChromeWindowTitleHasCafeManagement {
    ...
    foreach ($process in $chromeProcesses) {
        foreach ($pattern in $ChromeTitlePatterns) {
            if ($process.MainWindowTitle -and $process.MainWindowTitle.Contains($pattern)) {
                Write-Log "CafeManagement page detected by Chrome window title. PID: $($process.Id)"
                return $true
            }
        }
    }
...
function Test-ChromeTabTitleHasCafeManagement {
    try {
            ...
            foreach ($tab in $tabs) {
                ...
                foreach ($pattern in $ChromeTitlePatterns) {
                    if ($tabName -and $tabName.Contains($pattern)) {
                        Write-Log "CafeManagement page detected by Chrome tab title. PID: $($process.Id). Tab: $tabName"
                        return $true
                    }
                }
            }
...
function Test-CafeManagementAlreadyOpen {
    if (Test-ChromeCommandLineHasCafeManagement) {
        return $true
    }

    if (Test-ChromeWindowTitleHasCafeManagement) {
        return $true
    }

    if (Test-ChromeTabTitleHasCafeManagement) {
        return $true
    }
...
function Open-CafeManagement {
    if ($LaunchThroughExplorerShell) {
        Write-Log "Opening CafeManagement through Windows shell: $WebUiUrl"
    ...
    Write-Log "Opening CafeManagement in browser: $resolvedBrowserPath $WebUiUrl"
    Start-Process -FilePath $resolvedBrowserPath -ArgumentList $WebUiUrl
}

Write-Log "CafeManagement WatchDog started."
...
if (Test-CafeManagementAlreadyOpen) {
    Write-Log "CafeManagement is already open. Browser launch skipped."
    exit 0
}

Open-CafeManagement
Write-Log "CafeManagement WatchDog completed."
